...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # (N, T) -> (N, T, W)
        captions_embedding = self.positional_encoding(self.embedding(captions))

        # (N, D) -> (N, W) -> (N, 1, W)
        features_projected = self.visual_projection(features)[:, None, :]

        tgt_mask = torch.tril(torch.ones((T, T), dtype=captions.dtype, device=captions.device))

        output = self.transformer(captions_embedding, features_projected, tgt_mask)
