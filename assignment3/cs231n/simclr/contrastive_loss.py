...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l1 = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / torch.sum(torch.exp(sims_k / tau)))
        l2 = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / torch.sum(torch.exp(sims_k_N / tau)))
        total_loss += l1 + l2
...
def sim_positive_pairs(out_left, out_right):
    ...
    norm = torch.linalg.norm(out_left, dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1, keepdim=True)
    pos_pairs = torch.sum(out_left * out_right, dim=1, keepdim=True) / norm
...
def compute_sim_matrix(out):
    ...
    out_normed = out / torch.linalg.norm(out, dim=1, keepdim=True)
    sim_matrix = out_normed @ out_normed.t()
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = torch.exp(pos_pairs / tau).tile((2, 1))
