...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pow = torch.pow(10000, -torch.arange(0, embed_dim, 2) / embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        K = self.key(key).reshape(N, T, H, self.head_dim).moveaxis(1, 2)       # (N, H, T, E // H)
        Q = self.query(query).reshape(N, S, H, self.head_dim).moveaxis(1, 2)   # (N, H, S, E // H)
        V = self.value(value).reshape(N, T, H, self.head_dim).moveaxis(1, 2)   # (N, H, T, E // H)

        Y = Q @ K.transpose(2, 3) / math.sqrt(E / H)                           # (N, H, S, T)
        if attn_mask is not None:
            Y = Y.masked_fill(attn_mask == 0, float('-inf'))                   # (N, H, S, T)
        Y = self.attn_drop(F.softmax(Y, dim=-1)) @ V                           # (N, H, S, E // H)
        Y = Y.moveaxis(1, 2).reshape((N, S, E))                                # (N, S, E)
