...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for k, (i, j) in enumerate(zip([input_dim, *hidden_dims], [*hidden_dims, num_classes])):
            ...
            if self.normalization and k < len(hidden_dims):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            w, b, gamma, beta = (self.params.get(t, None) for t in keys)

            bn_param = self.bn_params[i] if self.normalization and i != self.num_layers - 1 else None
            dropout_param = self.dropout_param if self.use_dropout else None

            X, cache[i + 1] = universal_forward(X, w, b, gamma, beta, bn_param, dropout_param, i == self.num_layers - 1)
        ...
        for i in range(self.num_layers, 0, -1):
            dout, grads[f'W{i}'], grads[f'b{i}'], dgamma, dbeta = universal_backward(dout, cache[i])

            grads[f'W{i}'] += self.reg * self.params[f'W{i}']
            if dgamma is not None and i < self.num_layers:
